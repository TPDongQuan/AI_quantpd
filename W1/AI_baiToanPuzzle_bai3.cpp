...
int main(){
	...
	REPS(i,0,way.length()){
		if(way[i] == 'l'){
			moveLeft();
			prin();
			cout << "sang trái" << endl << endl;
		}else if(way[i] == 'r'){
			moveRight();
			prin();
			cout << "sang phải" << endl << endl;
		}else if(way[i] == 'u'){
			moveUp();
			prin();
			cout << "đi lên" << endl << endl;
		}else if(way[i] == 'd'){
			moveDown();
			prin();
			cout << "đi xuống" << endl << endl;
		}
	}
	cout << "Thuật toán BFS : " << endl;
	cout << "Số bước = " << way.length() << endl;
	cout << "Số phép toán đã thực hiện = " << numOfNode << endl;
	cout << "Thời gian tính toán = " << float( clock () - begin_time ) /  CLOCKS_PER_SEC << "s";
	return 0;
}