...
def march(board, y, x, dy, dx, length):
    '''
    tìm v? trí xa nh?t trong dy,dx trong kho?ng length
    '''
    yf = y + length * dy
    xf = x + length * dx
...
def winning_situation(sumcol):
    '''
    tr? l?i tình hu?ng chi?n th?ng d?ng nhu:
    {0: {}, 1: {(0, 1): 4, (-1, 1): 3, (1, 0): 4, (1, 1): 4}, 2: {}, 3: {}, 4: {}, 5: {}, -1: {}}
    1-5 luu di?m có d? nguy hi?m t? th?p d?n cao,
    -1 là roi vào tr?ng thái t?i, c?n phòng th?
    '''
...
def best_move(board, col):
    '''
    tr? l?i di?m s? c?a m?ng trong l?i th? c?a t?ng màu
    '''
    if col == 'w':
        anticol = 'b'
    else:
